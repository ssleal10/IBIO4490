...
feature_params = struct('template_size', 36, 'hog_cell_size', [3,6,9,12,18]);


% Step 1. Load positive training crops and random negative examples
%YOU CODE 'get  _positive_features' and 'get_random_negative_features'

features_pos = get_positive_features( train_path_pos, feature_params );
% NEGATIVE EXAMPLES
num_negative_examples = 50; %Higher will work strictly better, but you should start with 10000 for debugging
...
X = X';
Y = [ones(length(features_pos),1);-ones(length(features_neg),1)];
%
